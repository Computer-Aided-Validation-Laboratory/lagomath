...
test "VecStack direct vec field access and get set compatibility" {
    var vector = Vec3f{ .vec = .{ 10.0, 20.0, 30.0 } };
    try expectEqual(@as(f64, 10.0), vector.vec[0]);
    try expectEqual(@as(f64, 20.0), vector.vec[1]);
    try expectEqual(@as(f64, 30.0), vector.vec[2]);

    try expectEqual(vector.vec[0], vector.get(0));
    try expectEqual(vector.vec[1], vector.get(1));
    try expectEqual(vector.vec[2], vector.get(2));

    vector.vec[1] = 42.0;
    try expectEqual(@as(f64, 42.0), vector.get(1));

    vector.set(1, 99.0);
    try expectEqual(@as(f64, 99.0), vector.vec[1]);
}
